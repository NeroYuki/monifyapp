    ...
    new Promise(async (resolve, reject) => {
        ...
        for (let time of date) {
            ...
            for (let j of InTrans) {
                if (Math.floor(new Date(j.thoigian).getTime() / 86400000) == Math.floor(new Date(time).getTime() / 86400000)) {
                    data.push(JSON.parse(JSON.stringify(j)))
                }
            }
            if (data.length != 0) {
                OutTrans.trans.push({ time, data })
            }
        }
        resolve(OutTrans)
    })
